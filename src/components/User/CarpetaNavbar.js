...
const CarpetaNavbar = ({ carpeta, data, onFolderSelect, classifierData }) => {
    ...
    useEffect(() => {
        const obtenerCarpetasSeleccionadas = async () => {
            try {
                ...
                setSelectedNavbarFolders(carpetasSeleccionadas);
            } catch (error) {
    ...
    const toggleSeleccion = (e) => {
        ...
        const isSelected = selectedNavbarFolders.some((selected) => selected.IdFolder === carpeta.IdFolder);

        if (isSelected) {
            setSelectedNavbarFolders((prevSelected) => prevSelected.filter((selected) => selected.IdFolder !== carpeta.IdFolder));
        } else {
            setSelectedNavbarFolders((prevSelected) => [...prevSelected, carpeta]);
        }
    ...
    const eliminarCarpetaDeBaseDeDatos = async (idFolder) => {
        try {
            await axios.delete(url + `/eliminar-carpetanavbar/${idFolder}`);
        ...
        <div style={{ position: 'relative' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center' }}>
                <div className='icono-carpetas-clasificador' onClick={toggleAbierta}>
                    ...
                    <span className="texto-carpetas" style={{ fontWeight: selectedNavbarFolders.some((selected) => selected.IdFolder === carpeta.IdFolder) ? 'bold' : 'normal', color: 'black' }}>{carpeta.Name}</span>
                </div>
                <input type="checkbox" checked={selectedNavbarFolders.some((selected) => selected.IdFolder === carpeta.IdFolder)} onChange={toggleSeleccion} style={{ marginLeft: '10px', marginTop: '5px' }} />
            </div>
                    ...
                    {data
                        ...
                        .map(subcarpeta => (
                            <li key={subcarpeta.IdFolder}>
                                <CarpetaNavbar carpeta={subcarpeta} data={data} onFolderSelect={onFolderSelect} />
                            </li>
                        ))}
                </ul>
            )}
