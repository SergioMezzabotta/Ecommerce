...
const Carpeta = ({ carpeta, data, onFolderSelect, classifierData }) => {
    ...
    useEffect(() => {
        const obtenerCarpetasSeleccionadas = async () => {
            try {
                ...
                setSelectedFolders(carpetasSeleccionadas);
            } catch (error) {
    ...
    const toggleSeleccion = (e) => {
        ...
        const isSelected = selectedFolders.some((selected) => selected.IdFolder === carpeta.IdFolder);

        if (isSelected) {
            setSelectedFolders((prevSelected) => prevSelected.filter((selected) => selected.IdFolder !== carpeta.IdFolder));
        } else {
            setSelectedFolders((prevSelected) => [...prevSelected, carpeta]);
        }
    ...
    const eliminarCarpetaDeBaseDeDatos = async (idFolder) => {
        try {
            await axios.delete(url + `/eliminar-carpeta/${idFolder}`);
        ...
        <div style={{ position: 'relative' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center' }}>
                <div className='icono-carpetas-clasificador' onClick={toggleAbierta}>
                    ...
                    <span className="texto-carpetas" style={{ fontWeight: selectedFolders.some((selected) => selected.IdFolder === carpeta.IdFolder) ? 'bold' : 'normal', color: 'black' }}>{carpeta.Name}</span>
                </div>
                <input type="checkbox" checked={selectedFolders.some((selected) => selected.IdFolder === carpeta.IdFolder)} onChange={toggleSeleccion} style={{ marginLeft: '10px', marginTop: '5px' }} />
            </div>
                    ...
                    {data
                        ...
                        .map(subcarpeta => (
                            <li key={subcarpeta.IdFolder}>
                                <Carpeta carpeta={subcarpeta} data={data} onFolderSelect={onFolderSelect} />
                            </li>
                        ))}
                </ul>
            )}
