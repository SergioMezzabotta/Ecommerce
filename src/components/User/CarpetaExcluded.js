...
const CarpetaExcluded = ({ carpeta, data, onFolderSelect, classifierData }) => {
    const url = process.env.REACT_APP_BACK_URL;

    const [abierta, setAbierta] = useState(false);
    const [excludedFolders, setSelectedExcludedFolders] = useState([]);
    ...
    useEffect(() => {
        const obtenerCarpetasSeleccionadas = async () => {
            try {
                ...
                setSelectedExcludedFolders(carpetasSeleccionadas);
            } catch (error) {
    ...
    const toggleSeleccion = (e) => {
        ...
        const isSelected = excludedFolders.some((selected) => selected.IdFolder === carpeta.IdFolder);

        if (isSelected) {
            setSelectedExcludedFolders((prevSelected) => prevSelected.filter((selected) => selected.IdFolder !== carpeta.IdFolder));
        } else {
            setSelectedExcludedFolders((prevSelected) => [...prevSelected, carpeta]);
        }
    ...
    const eliminarCarpetaDeBaseDeDatos = async (idFolder) => {
        try {
            await axios.delete(url + `/eliminar-carpetasexcluidas/${idFolder}`);
        ...
        <div style={{ position: 'relative' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center' }}>
                <div className='icono-carpetas-clasificador' onClick={toggleAbierta}>
                    ...
                    <span className="texto-carpetas" style={{ fontWeight: excludedFolders.some((selected) => selected.IdFolder === carpeta.IdFolder) ? 'bold' : 'normal', color: 'black' }}>{carpeta.Name}</span>
                </div>
                <input type="checkbox" checked={excludedFolders.some((selected) => selected.IdFolder === carpeta.IdFolder)} onChange={toggleSeleccion} style={{ marginLeft: '10px', marginTop: '5px' }} />
            </div>
                    ...
                    {data
                        ...
                        .map(subcarpeta => (
                            <li key={subcarpeta.IdFolder}>
                                <CarpetaExcluded carpeta={subcarpeta} data={data} onFolderSelect={onFolderSelect} />
                            </li>
                        ))}
                </ul>
            )}
...
export default CarpetaExcluded;